...
def read_asc_file(filepath):
    ...
    with open(filepath, 'r') as file:
        ...
        data[data == asc_header['nodata_value']] = np.nan

    return data, asc_header


def write_asc_file(data, file_name, asc_header):
    """Writes data to an .asc file using the provided header information."""

    try:
        with open(file_name, 'w') as file:
            file.write(f"NCOLS {asc_header['ncols']}\n")
            file.write(f"NROWS {asc_header['nrows']}\n")
            file.write(f"XLLCORNER {asc_header['xllcorner']}\n")
            file.write(f"YLLCORNER {asc_header['yllcorner']}\n")
            file.write(f"CELLSIZE {asc_header['cellsize']}\n")
            file.write(f"NODATA_VALUE {asc_header['nodata_value']}\n")
...
def calculate_spei(D, asc_header, distribution="ll"):
...
def calc_spei_n_months(start_date: datetime, end_date: datetime, pet_equation: str = "fao56pm", distribution="ll", calc_1_month=True):
    ...
    T_min_asc_content, asc_header = load_multi_asc_files(
    ...
    if pet_equation.lower() == "fao56pm":
        ...
        while curr_date <= end_date:
            ...
            if months_diff == 1 or calc_1_month:
                # 1-month SPEI
                spei = calculate_spei(
                    D=D_results[curr_date_str], asc_header=asc_header, distribution=distribution)

                output_file_path = f"output/1month/spei_fao56pm_{distribution}_{curr_date_str}.asc"
                ok = write_asc_file(
                    data=spei, file_name=output_file_path, asc_header=asc_header)
                if ok:
                    print(
        ...
        if months_diff > 1:
            ...
            # Calculate SPEI
            spei = calculate_spei(
                D=D_sum, asc_header=asc_header, distribution=distribution)

            # Save n-month SPEI as asc file
            total_months = len(D_results)
            output_file_path = f"output/{total_months}month/spei_fao56pm_{distribution}_{curr_date_str}.asc"
            ok = write_asc_file(
                data=spei,
            ...
            plot_spei(spei, asc_header,
                      f"output/{total_months}month/spei_fao56pm_{distribution}_{curr_date_str}.png")


def plot_spei(spei_data, asc_header, save_path=""):
    ...
    x_min, y_min, x_max, y_max = transform_coordinates(
        asc_header['xllcorner'], asc_header['yllcorner'],
        asc_header['xllcorner'] +
        asc_header['ncols'] * asc_header['cellsize'],
        asc_header['yllcorner'] +
        asc_header['nrows'] * asc_header['cellsize'],
    ...
    plt.imshow(X=spei_data_masked,
               ...
               norm=spei_norm,
               extent=extent,
               # extent=[0, asc_header['ncols'], 0, asc_header['nrows']]
               )
    ...
    while current_date <= end_date:
        ...
        if exists(full_path):
            if asc_header:
                ...
                asc_file_content[current_date.strftime(
                    '%Y%m')], asc_header = read_asc_file(full_path)

        next_month = current_date.replace(day=28) + timedelta(days=4)
        current_date = next_month - timedelta(days=next_month.day - 1)

    return asc_file_content, asc_header
...
calc_spei_n_months(start_date=start, end_date=end,
                   distribution="g", calc_1_month=False)
calc_spei_n_months(start_date=start, end_date=end,
                   distribution="ll", calc_1_month=False)
